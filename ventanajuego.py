...
def movimientomapa():#mueve el mapa 
    global fondo,fondo2,carros,mancha,carros2,posxc,posyc,posxc2,posyc2
    can.move(fondo,0,20)
...
def enemigos():#funcion para el movimiento de los carros enemigos
    ...
    if(carron[1]==True):
        ...
        can.delete(carron[0])
        carron[0]=can.create_image(tmp[0],tmp[1]-15,image=enemigo1,anchor=NW)
    ...
    if(carron1[1]==True):
        ...
        can.delete(carron1[0])
        carron1[0]=can.create_image(tmp[0],tmp[1]-15,image=enemigo2,anchor=NW)
...
def nombres():
    import Inicio
    nombre1=Label(ventana,text=s.get()).place(x=200,y=200)
can.focus_set()
